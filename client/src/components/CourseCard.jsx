const CourseCard = ({ course, onClick }) => (
    ...
    className="bg-card text-card-foreground dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 border border-border cursor-pointer" onClick={onClick}>
        <img className="h-56 w-full object-cover transition duration-300 ease-in-out hover:scale-110" src={course.imageUrl} alt={course.title} onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/600x400/e2e8f0/e2e8f0?text=." }} />
        <div className="p-6">
